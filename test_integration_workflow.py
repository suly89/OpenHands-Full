...
def test_integration_workflow():
    ...
    try:
        ...
        state = State()
        initial_message = MessageAction(content="Create a simple web application that displays 'Hello World'", source='user')
        ...
        with patch.object(mock_llm, 'completion', return_value=create_mock_llm_response(
        ...
        dev_tasks = [t for t in BacklogTool.get_backlog_tasks() if t['phase'] == 'development']
        if dev_tasks:
        ...
        test_tasks = [t for t in BacklogTool.get_backlog_tasks() if t['phase'] == 'testing']
        if test_tasks:
        ...
        # Verify completion
        assert isinstance(action, AgentFinishAction), f"Expected AgentFinishAction, got {type(action)}"
        ...
        phases = ['requirements_gathering', 'planning', 'architecture', 'development', 'testing', 'validation']
        for phase in phases:
