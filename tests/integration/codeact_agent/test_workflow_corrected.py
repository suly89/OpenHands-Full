...
class TestWorkflow:
    ...
    def mock_llm(self):
        ...
        # Mock the completion to return a ModelResponse
        def mock_completion(*args, **kwargs):
            content = kwargs.get('content', 'Mock response')
    ...
    def test_planning_to_execution_transition(
        ...
        with (
            ...
            patch(
                'openhands.agenthub.codeact_agent.codeact_agent.CodeActAgent.prompt_manager',
                new_callable=PropertyMock(return_value=mock_prompt_manager),
            ),
        ):
            ...
            # Set the mock response before the step call
            ...
            # Add the LLM response to the state history
            llm_response = MessageAction(content='We are moving to development')
            llm_response._source = 'agent'
        ...
        with (
            ...
            state = State(
                history=[
                    ...
                    MessageAction(content='We are moving to development'),
                ]
            )
            state.history[0]._source = 'user'
            ...
            # Add the LLM response to the state history
            llm_response = MessageAction(content='Development complete')
            llm_response._source = 'agent'
            state.history.append(llm_response)
    ...
    def test_state_persistence_across_steps(self, agent, mock_llm, mock_prompt_manager):
        ...
        with (
            ...
            # Add the LLM response to the state history
            llm_response1 = MessageAction(content='We are moving to development')
            llm_response1._source = 'agent'
            state.history.append(llm_response1)
            ...
            # Add the LLM response to the state history
            llm_response2 = MessageAction(content='Development complete')
            llm_response2._source = 'agent'
            state.history.append(llm_response2)
        ...
        with (
            ...
            state = State(
                history=[
                    MessageAction(content='User request'),
                    MessageAction(content='We are moving to development'),
                    MessageAction(content='Development complete'),
                ]
            )
            state.history[0]._source = 'user'
            state.history[1]._source = 'agent'
            state.history[2]._source = 'agent'
            # Set the mock response before the step call
            mock_llm.completion.return_value = 'Mock response'
            agent.step(state)
